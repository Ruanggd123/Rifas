...
register_shutdown_function(function() {
    ...
    if ($error !== NULL && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
        ...
        echo json_encode(["status" => "failed", "error" => "FATAL: " . $error['message'] . " in " . $error['file'] . " line " . $error['line']]);
    }
});
...
class Main extends DBConnection
{
    ...
    public function add_to_card()
    {
        ...
        // Verifica se o cliente existe no banco antes de tentar inserir no carrinho
        $check_customer = $this->conn->query("SELECT id FROM customer_list WHERE id = '$customer_id'")->num_rows;
        if ($check_customer == 0) {
            // Destrói a sessão para forçar o deslogamento
            session_destroy();
            foreach ($_SESSION as $key => $value) {
                unset($_SESSION[$key]);
            }
            $resp["status"] = "failed";
            $resp["msg"] = "Sua sessão expirou. Por favor, atualize a página para fazer login novamente.";
            return json_encode($resp);
        }
    ...
    public function place_order()
    {
        ...
        $lockFile = sys_get_temp_dir() . '/pedido_' . md5(__DIR__) . '.lock';
        ...
        if (!$lock || flock($lock, LOCK_EX)) {
            ...
            if (!$quantity) {
                // DEBUG INFO
                $cart_check = $this->conn->query("SELECT * FROM cart_list WHERE customer_id = '$customer_id'")->fetch_all(MYSQLI_ASSOC);
                $resp["status"] = "failed";
                $resp["error"] = "Erro ao criar pedido. DEBUG: customer_id=$customer_id, cart_count=" . count($cart_check);
                flock($lock, LOCK_UN);
                fclose($lock);
                return json_encode($resp);
                exit();
...
switch ($action) {
    ...
    file_put_contents(dirname(__DIR__) . '/error_log.txt', "Error on action $action: " . $e->getMessage() . " in " . $e->getFile() . " on line " . $e->getLine() . "\n" . $e->getTraceAsString(), FILE_APPEND);
    http_response_code(500);
