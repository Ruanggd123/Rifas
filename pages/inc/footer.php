...
if ($enable_footer == '1') { ?>
	<style>
        ...
        @media (prefers-color-scheme: dark) {
            .container-fluid.rodape {
                background: rgba(15, 23, 42, 0.6); /* Deep dark blue glass */
		...
		ul.social a.whatsapp1:hover { color: #25D366; border-color: rgba(37, 211, 102, 0.5); }
		ul.social a.instagram1:hover { color: #E1306C; border-color: rgba(225, 48, 108, 0.5); }
		ul.social a.facebook1:hover { color: #1877F2; border-color: rgba(24, 119, 242, 0.5); }
		ul.social a.twitter1:hover { color: #1DA1F2; border-color: rgba(29, 161, 242, 0.5); }
		ul.social a.youtube1:hover { color: #FF0000; border-color: rgba(255, 0, 0, 0.5); }
		...
		<div class="container px-4">
            <ul class="social">
                <?php if ($whatsapp_footer) { ?>
                    <li class="spacing-icon">
                        <a class="whatsapp1" target="_blank" href="<?= $whatsapp_footer ?>" aria-label="WhatsApp">
                ...
                <?php if ($instagram_footer) { ?>
                    <li class="spacing-icon">
                        <a class="instagram1" target="_blank" href="<?= $instagram_footer ?>" aria-label="Instagram">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 16 16">
                ...
                <?php if ($facebook_footer) { ?>
                    <li class="spacing-icon">
                        <a class="facebook1" target="_blank" href="<?= $facebook_footer ?>" aria-label="Facebook">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 16 16">
                ...
                <?php if ($twitter_footer) { ?>
                    <li class="spacing-icon">
                        <a class="twitter1" target="_blank" href="<?= $twitter_footer ?>" aria-label="Twitter">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 16 16">
                ...
                <?php if ($youtube_footer) { ?>
                    <li class="spacing-icon">
                        <a class="youtube1" target="_blank" href="<?= $youtube_footer ?>" aria-label="YouTube">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 16 16">
            ...
            <div class="rodape-text">
                <?php if ($text_footer) {
                    ...
                    <p class="mb-1">
                        <span class="font-weight-600">© Copyright <?= date('Y') ?> - <?= $_settings->info('name') ?></span><br>
                        Todos os direitos reservados.
                    </p>
                    <p class="mt-2 mb-0" style="display: flex; flex-direction: column; align-items: center; gap: 0.8rem;">
                        <span>
                            Desenvolvido por<br>
                            <a href="<?= BASE_URL ?>" target="_blank" rel="noreferrer" class="developer-badge" style="background: linear-gradient(135deg, #10b981, #059669); box-shadow: 0 4px 10px rgba(16, 185, 129, 0.3);">
                                <?= $_settings->info('name') ?>
                            </a>
                        </span>
                        
                        <span style="font-size: 0.75rem; opacity: 0.9; display: flex; align-items: center; gap: 0.4rem;">
                            Aprimoramentos de UI/UX & Código por 
                            <a href="https://github.com/ruanggd123" target="_blank" rel="noreferrer" class="developer-badge" style="margin-top: 0; padding: 0.2rem 0.6rem; font-size: 0.7rem; background: linear-gradient(135deg, #334155, #0f172a); box-shadow: 0 4px 10px rgba(15, 23, 42, 0.4); display: flex; align-items: center; gap: 0.3rem;">
...
if (!$user_id) { ?>
					...
					<div class="modal-content rounded-0">
						...
						<div class="modal-body app-form">
							...
							<div class="mb-2">
								...
								<input type="text" name="firstname" class="form-control text-black" id="firstname2" placeholder="Nome" required="">
							</div>
							<div class="mb-2">
								<label for="lastname2" class="form-label text-white">Sobrenome</label>
								<input type="text" name="lastname" class="form-control text-black" id="lastname2" placeholder="Sobrenome" required="">
							</div>
							<?php
							if ($enable_cpf == 1) { ?>
								<div class="mb-2">
							...
							if ($enable_email == 1) { ?>
								<div class="mb-2">
									...
									<input type="email" name="email" class="form-control text-black" id="email2" placeholder="exemplo@exemplo.com" required>
								</div>
							<?php
							}
	...
	<script>
		...
		$(document).ready(function() {
			// Modal de cadastro padrão (quando não está no checkout)
			$('#form-cadastrar, #cadastroModal2').submit(function(e) {
				e.preventDefault();
				var form = $(this);
				var phoneValue = form.find('[name="phone"]').val() || '';
				var phoneConfirmValue = form.find('[name="phone_confirm"]').val() || '';
				...
				$.ajax({
					...
					success: function(resp) {
						...
						} else if (resp.status == 'phone_already' || resp.status == 'cpf_already' || resp.status == 'email_already') {
							alert(resp.msg);
						} else {
							alert('An error occurred');
							console.log(resp);
			...
			// Modal de Login padrão (quando não está no checkout)
					...
					success: function(resp) {
						...
						if (resp.status == 'success') {
							...
							<?php } else { ?>
								...
								$('#cadastroModal2 [name="phone"]').val(phone);
								$('#openCadastro').click();
							<?php } ?>
						} else {
							alert('An error occurred');
			...
			// Etapa 1 do checkout: Login do cliente
					...
					success: function(resp) {
						...
						if (resp.status == 'success') {
							<?php if (isset($slug)) { ?>
								...
								$('#step1-header i').removeClass('bi-search').addClass('bi-check');
								$('#step2-header i').removeClass('bi-person-add').addClass('bi-check');
								$('#step3-header i').addClass('active');
							<?php } else { ?>
								location.reload();
							<?php } ?>
						} else if (resp.status == 'failed' && resp.msg == 'Incorrect Phone Number') {
							// Se o telefone não for encontrado, encaminha para a Etapa 2 de cadastro no checkout
							var phone = form.find('[name="phone"]').val();
							$('#cadastroCheckoutForm [name="phone"]').val(phone);
							$('#step1').hide();
							$('#step2').show();
							$('.step1-progress').addClass('active');
							$('#step2-header i').addClass('active');
							...
							// Outros erros como senha incorreta
							alert(resp.msg);
						} else {
							alert('An error occurred');
						}
			...
			// Etapa 2 do checkout: Cadastro do cliente
			$('#cadastroCheckoutForm').submit(function(e) {
				e.preventDefault();
				var form = $(this);
				var phoneValue = form.find('[name="phone"]').val() || '';
				var phoneConfirmValue = form.find('[name="phone_confirm"]').val() || '';
			...
			// Navegação manual de etapas no checkout
			...
			// Submissão do formulário final de compra
...
// Auto-trigger cron job to expire orders (limited to 1x per minute on the server side)
setTimeout(() => { fetch(typeof _base_url_ !== 'undefined' ? _base_url_ + 'admin/cron.php' : '/admin/cron.php').catch(() => {}); }, 15000);
</script>
