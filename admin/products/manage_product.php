...
<?= isset($description) ? $description : '' ?>
                ...
                <div id="tab7" class="tabcontent text-gray-700 dark:text-gray-400 hidden">
                    ...
                    <label class="block mt-4 text-sm">
                        <div class="text-gray-700 dark:text-gray-400">Probabilidade de Saída da Cota Premiada (0-100%)</div>
                        <p style="font-size: 11px; color: orange;">Esta é a chance de uma cota premiada (tanto desta aba quanto da Roleta) sair automaticamente em uma compra.</p>
                        <input type="range" class="win10-thumb" name="probabilidade_roleta" min="0" max="100" value="<?= isset($probabilidade_roleta) ? $probabilidade_roleta : 0 ?>" id="probabilidadeCotasRange" />
                        <div id="probabilidadeCotasValue" class="text-gray-700 dark:text-gray-400 font-bold"><?= isset($probabilidade_roleta) ? $probabilidade_roleta : 0 ?></div>
                    </label>
                    <script>
                        document.addEventListener("DOMContentLoaded", function() {
                            const cotasRangeInput = document.getElementById('probabilidadeCotasRange');
                            const cotasValueDisplay = document.getElementById('probabilidadeCotasValue');
                            if(cotasRangeInput && cotasValueDisplay) {
                                cotasRangeInput.addEventListener('input', function() {
                                    cotasValueDisplay.textContent = cotasRangeInput.value;
                                    // Sincronizar com o slider da aba Roleta (Tab 9)
                                    const roletaRange = document.getElementById('probabilidadeRoletaRange');
                                    const roletaValue = document.getElementById('probabilidadeRoletaValue');
                                    if(roletaRange && roletaValue) {
                                        roletaRange.value = cotasRangeInput.value;
                                        roletaValue.textContent = cotasRangeInput.value;
                                    }
                                });
                            }
                        });
                ...
                <div id="tab9" class="tabcontent text-gray-700 dark:text-gray-400 hidden">
                    ...
                    <label class="block mt-4 text-sm">
                        <div class="text-gray-700 dark:text-gray-400">Probabilidade da Roleta Premiada (0-100%)</div>
                        <input type="range" class="win10-thumb" name="probabilidade_roleta" min="0" max="100" value="<?= isset($probabilidade_roleta) ? $probabilidade_roleta : 0 ?>" id="probabilidadeRoletaRange" />
                        <div id="probabilidadeRoletaValue" class="text-gray-700 dark:text-gray-400 font-bold"><?= isset($probabilidade_roleta) ? $probabilidade_roleta : 0 ?></div>
                    </label>

                    <script>
                        document.addEventListener("DOMContentLoaded", function() {
                            const roletaRangeInput = document.getElementById('probabilidadeRoletaRange');
                            const roletaValueDisplay = document.getElementById('probabilidadeRoletaValue');
                            if(roletaRangeInput && roletaValueDisplay) {
                                roletaRangeInput.addEventListener('input', function() {
                                    roletaValueDisplay.textContent = roletaRangeInput.value;
                                    // Sincronizar com o slider da aba Cotas Premiadas (Tab 7)
                                    const cotasRange = document.getElementById('probabilidadeCotasRange');
                                    const cotasValue = document.getElementById('probabilidadeCotasValue');
                                    if(cotasRange && cotasValue) {
                                        cotasRange.value = roletaRangeInput.value;
                                        cotasValue.textContent = roletaRangeInput.value;
                                    }
                                });
                            }
                        });
