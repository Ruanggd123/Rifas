...
class Master extends DBConnection
{
	...
	public function save_product()
	{
		...
		$cotas_premiadas_roleta = isset($_POST['cotas_premiadas_roleta']) ? $this->conn->real_escape_string($_POST['cotas_premiadas_roleta']) : '';
		$probabilidade_roleta = isset($_POST['probabilidade_roleta']) ? $this->conn->real_escape_string($_POST['probabilidade_roleta']) : 0;
...
//		return json_encode($resp);
	...
	public function place_order()
	{
		...
		$query = 'SELECT discount_qty, enable_discount, discount_amount, enable_cumulative_discount, enable_sale, sale_qty, sale_price, status, qty_numbers, pending_numbers, paid_numbers, date_of_draw, cotas_premiadas_roleta, probabilidade_roleta FROM product_list WHERE id = \'' . $product_id . '\'';
		...
		if ($result && 0 < $result->num_rows) {
			...
			$probabilidade_roleta = isset($row['probabilidade_roleta']) ? intval($row['probabilidade_roleta']) : 0;
			$cotas_premiadas_roleta_str = isset($row['cotas_premiadas_roleta']) ? $row['cotas_premiadas_roleta'] : '';
		}
		...
		if ($insert) {
			...
			if ($use_manual_numbers) {
				...
				// Probabilidade de Cota Premiada (Roleta)
				if ($probabilidade_roleta > 0 && !empty($cotas_premiadas_roleta_str)) {
					$roll = mt_rand(1, 100);
					if ($roll <= $probabilidade_roleta) {
						$roleta_numbers = explode(',', $cotas_premiadas_roleta_str);
						$available_roletas = [];
						foreach ($roleta_numbers as $rnum) {
							$rnum = trim($rnum);
							if ($rnum === '') continue;
							$rnum_padded = str_pad($rnum, $globos, "0", STR_PAD_LEFT);
							if (!in_array($rnum_padded, $numeros_ja_vendidos)) {
								$available_roletas[] = $rnum_padded;
							}
						}
						if (count($available_roletas) > 0) {
							$chosen_roleta = $available_roletas[array_rand($available_roletas)];
							if (!in_array($chosen_roleta, $order_numbers)) {
								$order_numbers[0] = $chosen_roleta;
							}
						}
					}
				}
		...
		if ($this->settings->info('enable_pixel') == 1) {
			$dados = ['first_name' => $customer_fname, 'last_name' => $customer_lname, 'phone' => (strlen($customer_phone) <= 11 ? '55' . $customer_phone : $customer_phone), 'id' => $oid, 'total_amount' => $total_amount];
	...
	public function send_order_whatsapp($phone, $name, $pname, $cotas, $total, $message, $pix, $order_token)
{
    ...
    if (!empty($message)) {
        ...
        $dwapi = [
            'number' => (strlen($phone) <= 11 ? '55' . $phone : $phone),
            'body' => $message
        ];
